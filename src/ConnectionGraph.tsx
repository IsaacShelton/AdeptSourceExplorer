...
export function ConnectionGraph() {
    ...
    useEffect(() => {
        ...
        const simulation = d3
            ...
            .force(
                'charge',
                d3.forceManyBody().strength((d: any) => d.data.charge)
            )
            .force('x', d3.forceX())
            .force('y', d3.forceY());
        ...
        <>
            ...
            <div className="absolute w-full h-full">
                <span>
                    <div
                        ref={svgRef}
                        className="flex justify-center m-0 p-0 overflow-hidden max-w-full max-h-full"
                    />
                    <div className="text-white bottom-0 fixed select-none bg-[#1C1C1C] w-full min-h-8">
                        <span
                            className="transition-opacity duration 500"
                            style={{ opacity: hoveredActive ? 1 : 0 }}
                        >
                            <span className="font-mono text-[#888888] pl-[10px]">
                                {hoveredData?.filename &&
                                    path.dirname(hoveredData.filename) + path.sep}
                            </span>
                            ...
                            <span className="font-mono text-[#888888] pr-[10px] float-right">
                                {hoveredData?.children &&
                                    hoveredData.children.length.toString() +
                                        ' ' +
                                        unit +
                                        plural(hoveredData.children)}
                            </span>
                        </span>
                    </div>
                </span>
